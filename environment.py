...
class Environment:
    ...
    def make_object_grid(self, N, M, na, nb):
        self.object_grid = [["0" for col in range(M)] for row in range(N)]
        self.init_objects(na, nb)

    def init_objects(self, na, nb):
        full_grid = [(row, col) for row in range(N) for col in range(M)]
    ...
    def is_valid_move(self, position, displacement):
        ...
        if row + drow >= N or row + drow < 0:
            return False

        if col + dcol >= M or col + dcol < 0:
            return False
